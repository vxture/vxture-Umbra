...
_usage() {
  ...
  echo "    all                                Full deployment pipeline"
...
case "$CMD" in
  ...
  # -- New canonical names -------------------------------------------------------
  environment)
    exec bash "$SCRIPT_DIR/deploy/00-check-environment.sh"
    ;;

  directories)
    exec bash "$SCRIPT_DIR/deploy/01-init-data-directories.sh"
    ;;

  reality-keys)
    exec bash "$SCRIPT_DIR/deploy/02-generate-reality-keys.sh"
    ;;

  certificates)
    exec bash "$SCRIPT_DIR/deploy/03-issue-tls-certificates.sh"
    ;;

  config)
    log_banner "Umbra - Render Configs"
    python3 "$SCRIPT_DIR/deploy/04-render-configuration-templates.py"
    echo ""
    check_rendered_nginx_cert_paths
    echo ""
    ...
    if docker ps --format '{{.Names}}' 2>/dev/null | grep -q "^${NGINX_CONTAINER}$"; then
      ...
      log_warn "Start services with: bash scripts/deploy.sh start"
    fi
    ;;

  start)
    exec bash "$SCRIPT_DIR/deploy/05-start-docker-services.sh"
    ;;

  verify)
    exec bash "$SCRIPT_DIR/deploy/06-verify-deployment.sh"
    ;;

  wizard)
    exec bash "$SCRIPT_DIR/deploy/07-post-deploy-wizard.sh"
    ;;

  # -- Legacy aliases -------------------------------------------------------------
  check)
    log_warn "'check' is deprecated. Use: bash scripts/deploy.sh environment"
    exec bash "$SCRIPT_DIR/deploy/00-check-environment.sh"
    ;;

  dirs)
    log_warn "'dirs' is deprecated. Use: bash scripts/deploy.sh directories"
    exec bash "$SCRIPT_DIR/deploy/01-init-data-directories.sh"
    ;;

  keys)
    log_warn "'keys' is deprecated. Use: bash scripts/deploy.sh reality-keys"
    exec bash "$SCRIPT_DIR/deploy/02-generate-reality-keys.sh"
    ;;

  certs)
    log_warn "'certs' is deprecated. Use: bash scripts/deploy.sh certificates"
    exec bash "$SCRIPT_DIR/deploy/03-issue-tls-certificates.sh"
    ;;

  up)
    log_warn "'up' is deprecated. Use: bash scripts/deploy.sh start"
    exec bash "$SCRIPT_DIR/deploy/05-start-docker-services.sh"
    ;;

  post)
    log_warn "'post' is deprecated. Use: bash scripts/deploy.sh wizard"
    exec bash "$SCRIPT_DIR/deploy/07-post-deploy-wizard.sh"
    ;;
