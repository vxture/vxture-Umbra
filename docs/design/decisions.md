...
Keys generated by [`02-generate-reality-keys.sh`](../../scripts/deploy/02-generate-reality-keys.sh) and stored in `DATA_DIR/private/reality.json`:
...
|--------|-------------------|
| `00-check-environment.sh` | Read-only validation, always safe |
| `01-init-data-directories.sh` | `mkdir -p` everywhere, safe |
| `02-generate-reality-keys.sh` | Skip if `reality.json` exists |
| `03-issue-tls-certificates.sh` | Reuses trusted LE certs that are not near expiry; removes only invalid zero-byte renewal configs |
| `04-render-configuration-templates.py` | Overwrites rendered configs (templates are source of truth) |
| `05-start-docker-services.sh` | `docker compose up -d` is idempotent |
| `06-verify-deployment.sh` | Read-only, always safe |
