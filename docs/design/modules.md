...
1. REALITY keys generated by [`02-generate-reality-keys.sh`](../../scripts/deploy/02-generate-reality-keys.sh)
...
SQLite databases are backed up via file copy or root-container archives in `scripts/ops/backup.sh`. Vaultwarden's full `data/` directory is archived (includes attachments and sends). Account portal state is archived as `account-data-<timestamp>.tar.gz`.
...
Real-certificate upgrade must use `bash scripts/ops.sh certs --upgrade`. It normalizes legacy workdirs, copies existing certificate state into `DATA_DIR/letsencrypt.staged`, reuses trusted LE certs that are not near expiry, issues only missing/expiring/non-trusted domains in staging, keeps partial staged successes after a failed run, verifies all staged domains independently for issuer, expiry, and name match, and activates the staged directory only after every domain succeeds.
