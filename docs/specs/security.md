...
`vpn.ruyin.ai` is the user-facing account portal. Users can only register with a one-time invite generated by an admin at `console.ruyin.ai/invites/`.
...
The account dashboard refreshes the current Marzban `subscription_url` server-side for the bound user code before rendering. Marzban admin credentials are available only inside the account service container; the browser receives only the user's subscription URL and session cookie.
