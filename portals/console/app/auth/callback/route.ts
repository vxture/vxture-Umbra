...
const OIDC_INVITE_RE = /^[A-Za-z0-9-]{1,64}$/;
 ...
 * vx_rp_session cookie. Active only when OIDC is configured and `code` is
 * present; the legacy `token` handoff below remains the fallback until cutover.
 */
async function oidcCallback(request: NextRequest, cfg: OidcConfig): Promise<NextResponse> {
  ...
  const invite = OIDC_INVITE_RE.test(authReq.invite) ? authReq.invite : "";
...
export async function GET(request: NextRequest) {
  // Prefer the OIDC flow when configured and the IdP returned an auth code.
  const oidcCfg = getOidcConfig();
  if (oidcCfg && request.nextUrl.searchParams.get("code")) {
    return oidcCallback(request, oidcCfg);
  }
